...
PROFESSOR_DEFAULT_PASSWORD = "prof123"
...
def get_or_create_professor(
    ...
    professor = session.exec(select(User).where(User.email == normalized_email)).first()

    if professor is not None:
        professor.first_name = first_name
        professor.last_name = last_name
        professor.password_hash = hash_password(PROFESSOR_DEFAULT_PASSWORD)
    ...
    professor = User(
        ...
        password_hash=hash_password(PROFESSOR_DEFAULT_PASSWORD),
...
def get_or_create_demo_user(
    ...
    if user is not None:
        ...
        if role in (UserRole.ADMIN, UserRole.STUDENT):
            user.office_id = None
    ...
    for subject_data in SUBJECTS:
        ...
        for professor_email in subject_data["professor_emails"]:
            professor = session.exec(
                select(User).where(User.email == normalize_email(professor_email))
            ).first()
